...
function App() {
  ...
  const [invitations, setInvitations] = useState<import("./lib/api").GroupInvitation[]>([]);
  ...
  useEffect(() => {
    ...
    if (!token) return;
    api.acceptInvitationByToken(token).then(async () => { window.history.replaceState({}, "", window.location.pathname); await refreshGroups(); await refreshInvitations(); notify("Invitation accepted. You are now a group member."); }).catch((requestError) => notify(requestError instanceof Error ? requestError.message : "Could not accept this invitation."));
  }, [authUser]);

  const addExpense = async (expense: Expense) => {
    if (authUser && /^\d+$/.test(activeGroup.id)) {
      try {
        await api.createExpense({ group: Number(activeGroup.id), title: expense.title, category: expense.category, amount: expense.amount.toFixed(2), payer: expense.backendPayerId ?? authUser.id, note: expense.note, occurred_on: new Date().toISOString().slice(0, 10), split_mode: expense.status === "Confirmed" ? "equal" : "exact", participants: expense.backendParticipants?.map((participant) => ({ user: participant.user, share_amount: participant.share_amount, share_value: 0 })) });
  ...
  return <div className={`app-shell ${theme}`}>
    ...
    {showExpense && <ExpenseModal onClose={() => setShowExpense(false)} onSave={addExpense} memberOptions={availableGroups.find((group) => group.id === activeGroup.id)?.members_detail ?? []} currentUserId={authUser?.id ?? 0} />}
    {showGroupCreate && <GroupCreateModal onClose={() => setShowGroupCreate(false)} onCreate={createGroup} />}
    {showInvite && <InviteModal group={activeGroup} invitations={invitations} onClose={() => setShowInvite(false)} onInvite={async (username) => { const result = await api.createInvitation({ group: Number(activeGroup.id), username }); await refreshInvitations(); notify(`Invitation sent to ${result.invitee_username}.`); }} onAccept={async (id) => { await api.acceptInvitation(id); await refreshGroups(); await refreshInvitations(); notify("Invitation accepted and membership updated."); }} onDecline={async (id) => { await api.declineInvitation(id); await refreshInvitations(); notify("Invitation declined."); }} />}
...
function GroupCreateModal({ onClose, onCreate }: { onClose: () => void; onCreate: (payload: { name: string; emoji: string; description: string }) => Promise<void> }) {
  ...
  return <div className="auth-backdrop" onClick={onClose}><section className="auth-modal" onClick={(event) => event.stopPropagation()}><button className="auth-close icon-button" onClick={onClose}><X size={17} /></button><div className="auth-mark"><Users size={19} /></div><span className="muted-label">NEW SHARED SPACE</span><h2>Create a group</h2><p className="auth-subtitle">Start a real BDT workspace, then invite people by username.</p><form onSubmit={submit}><label>Group name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Dhanmondi house" required /></label><div className="auth-grid"><label>Icon<input value={emoji} onChange={(event) => setEmoji(event.target.value)} maxLength={2} /></label><label>Description<input value={description} onChange={(event) => setDescription(event.target.value)} placeholder="What are you sharing?" /></label></div>{error && <div className="auth-error"><CircleDollarSign size={15} />{error}</div>}<button className="primary-button auth-submit" disabled={busy}>{busy ? "Creating…" : "Create group"}<ArrowUpRight size={15} /></button></form></section></div>;
}

function InviteModal({ group, invitations, onClose, onInvite, onAccept, onDecline }: { group: Group; invitations: import("./lib/api").GroupInvitation[]; onClose: () => void; onInvite: (username: string) => Promise<void>; onAccept: (id: number) => Promise<void>; onDecline: (id: number) => Promise<void> }) {
  const [search, setSearch] = useState(""); const [suggestions, setSuggestions] = useState<import("./lib/api").DirectoryUser[]>([]); const [busy, setBusy] = useState(false); const [message, setMessage] = useState("");
  useEffect(() => { if (!search.trim()) { setSuggestions([]); return; } const timer = window.setTimeout(() => { void api.directoryUsers(search).then(setSuggestions).catch(() => setSuggestions([])); }, 250); return () => window.clearTimeout(timer); }, [search]);
  const invite = async (username: string) => { setBusy(true); setMessage(""); try { await onInvite(username); setSearch(""); setSuggestions([]); setMessage(`Invitation sent to @${username}.`); } catch (requestError) { setMessage(requestError instanceof Error ? requestError.message : "Could not send invitation."); } finally { setBusy(false); } };
  const copyLink = async (token: string) => { await navigator.clipboard?.writeText(`${window.location.origin}/?invite=${token}`); setMessage("Invitation link copied. The invited user must sign in as the matching username."); };
  const received = invitations.filter((item) => item.invitee_username && item.status === "pending");
  return <div className="auth-backdrop" onClick={onClose}><section className="auth-modal" onClick={(event) => event.stopPropagation()}><button className="auth-close icon-button" onClick={onClose}><X size={17} /></button><div className="auth-mark"><Users size={19} /></div><span className="muted-label">GROUP INVITATIONS</span><h2>Invite to {group.name}</h2><p className="auth-subtitle">Search every active SplitWise+ account by username. Invitations are delivered to the recipient’s inbox.</p><label>Search username<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="tisha_bd" autoComplete="off" /></label>{suggestions.length > 0 && <div className="invite-suggestions">{suggestions.map((user) => <button key={user.id} onClick={() => void invite(user.username)}><span className="avatar avatar-sm">{user.initials}</span><span><strong>{user.display_name}</strong><small>@{user.username}</small></span><ArrowUpRight size={15} /></button>)}</div>}{message && <div className="auth-footnote"><Check size={13} /> {message}</div>}<div className="invite-list"><span className="muted-label">YOUR INVITATIONS</span>{invitations.length === 0 && <small>No invitations yet.</small>}{invitations.map((item) => <div className="invite-row" key={item.id}><span><strong>{item.group_name}</strong><small>{item.inviter_name} → @{item.invitee_username} · {item.status}</small></span>{item.status === "pending" && item.invitee_username && <span><button className="secondary-button small" onClick={() => void onAccept(item.id)}>Accept</button><button className="text-button" onClick={() => void onDecline(item.id)}>Decline</button></span>}{item.status === "pending" && <button className="icon-button" onClick={() => void copyLink(item.token)} title="Copy invitation link"><Copy size={15} /></button>}</div>)}</div><button className="secondary-button" onClick={onClose}>Done</button></section></div>;
}

function InvitationInbox({ invitations, onAccept, onDecline }: { invitations: import("./lib/api").GroupInvitation[]; onAccept: (id: number) => Promise<void>; onDecline: (id: number) => Promise<void> }) { const pending = invitations.filter((item) => item.status === "pending"); return <div className="notification-popover"><div><span className="muted-label">INVITATION INBOX</span></div>{pending.length === 0 && <small>No pending group invitations.</small>}{pending.map((item) => <div className="invite-row" key={item.id}><span><strong>{item.group_name}</strong><small>{item.inviter_name} invited you</small></span><span><button className="secondary-button small" onClick={() => void onAccept(item.id)}>Join</button><button className="text-button" onClick={() => void onDecline(item.id)}>Decline</button></span></div>)}</div>; }
...
function ExpenseModal({ onClose, onSave, memberOptions, currentUserId }: { onClose: () => void; onSave: (expense: Expense) => void; memberOptions: { user_id: number; name: string; initials: string; role: string }[]; currentUserId: number }) {
  ...
  const [payer, setPayer] = useState(memberOptions.find((member) => member.user_id === currentUserId)?.name || memberOptions[0]?.name || "Rafi");
  const [note, setNote] = useState("");
  const [receiptAttached, setReceiptAttached] = useState(false);
  const [participants, setParticipants] = useState(() => (memberOptions.length ? memberOptions.map((member) => member.name) : ["Rafi", "Tisha", "Nabil", "Mahi"]));
  const parsedAmount = Number(amount) || 0;
  const perPerson = parsedAmount / Math.max(participants.length, 1);
  const canSave = title.trim().length > 1 && parsedAmount > 0 && participants.length > 0;
  const categories = [
  ...
  const people = memberOptions.length ? memberOptions.map((member) => member.name) : ["Rafi", "Tisha", "Nabil", "Mahi", "Shuvo"];
  ...
  return <div className="modal-backdrop expense-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><form className="expense-modal expense-modal-modern" onSubmit={(event) => { event.preventDefault(); if (!canSave) return; onSave({ id: crypto.randomUUID(), title: title.trim(), category, amount: parsedAmount, payer, date: "Just now", note: note.trim() || (mode === "Equal" ? "Split equally with the group" : `${mode} split`), receipt: receiptAttached, status: "Confirmed", backendPayerId: memberOptions.find((member) => member.name === payer)?.user_id ?? currentUserId, backendParticipants: participants.map((name) => ({ user: memberOptions.find((member) => member.name === name)?.user_id ?? currentUserId, share_amount: (mode === "Equal" ? perPerson : perPerson) })) }); }}><div className="expense-modal-head"><div className="expense-kicker"><span className="expense-kicker-dot" /> NEW SHARED EXPENSE</div><button type="button" className="modal-close-button" onClick={onClose} aria-label="Close expense modal"><X size={18} /></button><h2>Make the split feel easy.</h2><p>Add one expense and we’ll keep the group balance clear.</p></div><div className="expense-modal-body"><div className="expense-form-column"><label className="expense-field expense-title-field"><span>What did you pay for?</span><div className="expense-input-shell"><Receipt size={17} /><input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} placeholder="e.g. Dinner at Dhanmondi" /></div></label><div className="expense-two-col"><label className="expense-field"><span>Amount</span><div className="expense-input-shell amount-shell"><b>৳</b><input value={amount} onChange={(event) => setAmount(event.target.value.replace(/[^0-9.]/g, ""))} inputMode="decimal" placeholder="0.00" /></div></label><label className="expense-field"><span>Paid by</span><div className="expense-select-shell"><Avatar member={members.find((member) => member.name === payer) || members[0]} size="sm" /><select value={payer} onChange={(event) => setPayer(event.target.value)}>{people.map((person) => <option key={person}>{person}</option>)}</select><ChevronDown size={14} /></div></label></div><div className="expense-section-label"><span>Choose a category</span><small>Helps your group understand the spend</small></div><div className="expense-category-grid">{categories.map((item) => <button type="button" key={item.name} className={`expense-category-choice ${category === item.name ? "selected" : ""}`} onClick={() => setCategory(item.name)}><span>{item.icon}</span><strong>{item.name}</strong><small>{item.hint}</small></button>)}</div><div className="expense-section-label"><span>Who is included?</span><small>{participants.length} of {people.length} people selected</small></div><div className="participant-picker">{people.map((person) => { const member = members.find((item) => item.name === person) || members[0]; const selected = participants.includes(person); return <button type="button" key={person} className={`participant-chip ${selected ? "selected" : ""}`} onClick={() => toggleParticipant(person)}><Avatar member={member} size="sm" /><span>{person}</span>{selected ? <Check size={13} /> : <Plus size={13} />}</button>; })}</div><div className="expense-section-label split-label"><span>How should this split?</span><small>Change this later if the group decides differently</small></div><div className="split-mode-grid">{[{ name: "Equal", icon: <Users size={16} />, copy: "Everyone pays the same" }, { name: "Exact", icon: <WalletCards size={16} />, copy: "Set each person’s amount" }, { name: "Percentage", icon: <Split size={16} />, copy: "Split by contribution" }].map((item) => <button type="button" key={item.name} className={`split-mode-choice ${mode === item.name ? "selected" : ""}`} onClick={() => setMode(item.name)}>{item.icon}<span><strong>{item.name}</strong><small>{item.copy}</small></span>{mode === item.name && <Check size={14} />}</button>)}</div><label className="expense-field expense-note-field"><span>Note <em>Optional</em></span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Add a little context for the group…" rows={2} /></label><button type="button" className={`receipt-upload ${receiptAttached ? "attached" : ""}`} onClick={() => setReceiptAttached((value) => !value)}><span className="receipt-upload-icon">{receiptAttached ? <Check size={16} /> : <Paperclip size={16} />}</span><span><strong>{receiptAttached ? "Receipt attached" : "Attach a receipt"}</strong><small>{receiptAttached ? "Ready to keep the expense trustworthy" : "Optional · JPG, PNG or PDF"}</small></span><ArrowUpRight size={15} /></button></div><aside className="expense-preview-panel"><div className="preview-eyebrow">LIVE SPLIT PREVIEW</div><div className="preview-orb"><Sparkles size={18} /></div><h3>{title.trim() || "Your new expense"}</h3><p>{category} · paid by {payer}</p><div className="preview-total"><small>Total</small><strong>{money(parsedAmount)}</strong></div><div className="preview-divider" /><div className="preview-split-row"><span><Users size={15} /> {participants.length} people</span><strong>{money(perPerson)} each</strong></div><div className="preview-members">{participants.slice(0, 4).map((person) => <Avatar key={person} member={members.find((member) => member.name === person) || members[0]} size="sm" />)}{participants.length > 4 && <span className="preview-more">+{participants.length - 4}</span>}</div><div className="preview-tip"><Sparkles size={14} /><span>Everyone sees the same story, from payment to settlement.</span></div></aside></div><div className="expense-modal-footer"><button type="button" className="modal-cancel" onClick={onClose}>Cancel</button><button className="expense-save-button" type="submit" disabled={!canSave}><span>{canSave ? "Save expense" : "Add a title and amount"}</span><ArrowUpRight size={16} /></button></div></form></div>;
}
